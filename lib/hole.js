...
function getHoleMessage(border, counts, clickPos, last) {
	...
	return [MessageType.HOLE, clickPos[0], clickPos[1], start[0], start[1], last, directions];
}


function openBorder(boardState, holeMessage) {
	boardState = boardState.map(arr => [...arr]);
		...
		for (let x_ = Math.max(x - 1, 0); x_ <= mx; x_++) {
			for (let y_ = Math.max(y - 1, 0); y_ <= my; y_++) {
				if (!filtered.some(p => p[0] === x_ && p[1] === y_)
                    && (boardState[y_][x_] === 9 || boardState[y_][x_] === 0))
					queue.unshift([x_, y_]);
			}
		}
	}
...
module.exports = { getHoleMessage, openBorder, openHole };